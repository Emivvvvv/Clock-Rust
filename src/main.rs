...
use chrono::{DateTime, Duration as ChronoDuration, Utc, Local, Timelike, TimeZone};
use clap::{App, Arg};
use std::mem::zeroed;
...
fn check_time() -> Result<f64, std::io::Error> {
    ...
    for time in &times {
        ...
        let weight = 1_000_000. / (delay * delay) ;
        if weight.is_finite() {
            offsets.push(offset);
    ...
    let avg_ofset = weighted_mean(&offsets, &offset_weights);
...
fn main() {
    let app = App::new("clock")
        ...
        .after_help(
            ...
            Junary 1970 0:00:00 UTC. For more accuracy, use another \
            format.",
        )
        .arg(
